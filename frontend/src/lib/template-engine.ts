...
export class TemplateExpressionEngine {
  ...
  private static safeEvaluate(expression: string, context: EvaluationContext): any {
    try {
      ...
      // Handle complex expressions like resultvar[0].name
      if (expression.includes('[') && expression.includes(']')) {
        // Parse array access: resultvar[0].name
        const arrayMatch = expression.match(/^([^\[]+)\[([^\]]+)\](.*)$/);
        if (arrayMatch) {
          const [, arrayName, indexExpr, remainingPath] = arrayMatch;
          const arrayData = safeContext[arrayName.trim()];
          
          if (Array.isArray(arrayData)) {
            const index = parseInt(indexExpr.trim(), 10);
            let current = arrayData[index];
            
            // Handle remaining path like .name
            if (remainingPath && current) {
              const props = remainingPath.split('.').filter((p: string) => p);
              for (const prop of props) {
